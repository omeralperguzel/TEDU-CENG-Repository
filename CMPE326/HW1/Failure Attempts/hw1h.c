...
// Function to print the ratings for a specific user
void print_user_ratings(RatingNode *head, int user_id) {
    RatingNode *node = head;
    
    // Traverse the list and print the ratings for the given user
    while (node != NULL) {
        if (node->user_id == user_id) {
            printf("(%d, %d, %.1f)\n", node->user_id, node->item_id, node->value);
        }
        node = node->next;
    }
}

// Function to print the ratings for a specific item
void print_item_ratings(RatingNode *head, int item_id) {
    RatingNode *node = head;
    
    // Traverse the list and print the ratings for the given item
    while (node != NULL) {
        if (node->item_id == item_id) {
            printf("(%d, %d, %.1f)\n", node->user_id, node->item_id, node->value);
        }
...
// Function to print the ratings for all users and items
void print_ratings(RatingNode *head) {
    RatingNode *node = head;
    
    // Traverse the list and print the ratings for all users and items
    while (node != NULL) {
        printf("(%d, %d, %.1f)\n", node->user_id, node->item_id, node->value);
        node = node->next;
    }
...
void free_list(RatingNode *head) {
    RatingNode *node = head;
    RatingNode *next;
