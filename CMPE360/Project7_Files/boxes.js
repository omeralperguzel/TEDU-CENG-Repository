...
//This variable is needed to store the WebGL rendering context
var gl;
...
for (var i =0; i < wireCube2Lookups.length; i++)
{
   shapes.wireCube2.points.push(cubeVerts[wireCube2Lookups[i]]);
...
//It loads data into points and colors arrays. Runs once as page loads.
var points = [];
var colors = [];
...
window.onload = function init() {
  ...
  // Load shaders and initialize attribute buffers for shapes.
  var program = initShaders(gl, "shader.vert", "shader.frag");
  gl.useProgram(program);

  // Set up data to draw the axes in their own VAO - runs once as page loads.
...
function render() {
	...
	//We are setting up the camera in this view.
	var eye = vec3(0.0, 0.0, 10.0);
	var at =  vec3(0.0, 0.0, 0.0);
	var up =  vec3(0.0, 1.0, 0.0);

   //We are setting up the orthogonal projection.
   var orthoLeft = -3.0;  
   var orthoRight = 3.0;  
   var orthoBottom = -3.0;
   var orthoTop = 3.0;    
   ...
   //We are drawing the axes in this view.
	mv = lookAt(eye,at,up);
   //mv = translate(0, 0, -15);
   ...
   // We are drawing the first cube in this part
   mv = mult(mv, translate(1, 0, 0));  // Translate to (1, 0, 0)
   gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
   gl.drawArrays(shapes.wireCube.type, shapes.wireCube.start, shapes.wireCube.size);


   // We are drawing the second cube in this part
